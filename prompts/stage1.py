...
def get_prompt1_outline(knowledge_point, duration=5, reference_image_path=None):
    ...
    ### 【重要】content 是 TTS 语音朗读稿，必须遵守以下规则：
    
    #### 绝对禁止（会导致 TTS 读错）：
    - ❌ LaTeX 数学表达式：$a^2$、\\frac{{1}}{{2}}、\\sqrt{{x}}
    - ❌ Markdown 格式：**加粗**、*斜体*、`代码`
    - ❌ 特殊符号作为数学符号：^、_、{{}}、\\
    
    #### 数学表达式的正确写法（用自然语言朗读）：
    - ❌ "$a^m ÷ a^n = a^{{m-n}}$" → ✅ "a的m次方除以a的n次方，等于a的m减n次方"
    - ❌ "$a^0 = 1$" → ✅ "a的零次方等于1"
    - ❌ "$a^{{-3}}$" → ✅ "a的负3次方"
    - ❌ "$\\frac{{1}}{{a^3}}$" → ✅ "a的3次方分之一"
    - ❌ "2a²+3a²=5a²" → ✅ "2a平方加3a平方等于5a平方"
    - ❌ "a² ÷ a = a" → ✅ "a平方除以a等于a"
    - ❌ "3x+2x=5x" → ✅ "3x加2x等于5x"
    
    #### 其他表达转换：
    - 分数：用"几分之几"表达，如 "3分之1"、"a分之1"
    - 根号：用"根号几"表达，如 "根号2"、"根号x"
    - 下标：用自然语言，如 "a1"读作"a一"
    - 不等号：用"大于"、"小于"、"不等于"
    ...
    ### content 示例：
    ❌ 错误示例（含 LaTeX，TTS 无法正确朗读）：
    "content": "这里有两个特殊情况。第一个，当指数相减等于零的时候，比如 $a^3 ÷ a^3 = a^0$，结果等于1。"

    ✅ 正确示例（纯文本，TTS 友好）：
    "content": "这里有两个特殊情况，考试特别爱考，你得记牢。第一个，当指数相减等于零的时候，比如a的3次方除以a的3次方，等于a的0次方，这个结果等于1。记住啊，任何不等于零的数的零次幂都等于1。第二个，当被减的指数更大时，结果是负指数，比如a的2次方除以a的5次方，等于a的负3次方，这个等于a的3次方分之一。这两个公式你现在可能用得少，但一定要知道，后面会经常碰到。"

    ❌ 错误示例（太干巴巴，像背书）：
    "content": "轴对称就一句话：沿某条直线对折，两边能完全重合。这条直线叫对称轴。"
